...
def decode_decimal_bytes(src: IO[bytes], precision: int, scale:int=0) -> decimal.Decimal:
...
def decode_decimal_fixed(src: IO[bytes], size: int, precision: int, scale:int=0) -> decimal.Decimal:
    raw_bytes = src.read(size)
    return _deserialize_decimal(raw_bytes, precision, scale)

def encode_decimal_fixed(msg: decimal.Decimal, size: int, precision: int, scale:int=0) -> bytes:
    _validate_decimal(msg, precision, scale)
    unscaled = _unscale_decimal(msg)
...
def _validate_decimal(d: decimal.Decimal, precision: int, scale:int) -> int:
    """
    ...
    if len(digits) > precision:
        raise ValueError("decimal value has more digits than is legal according to the schema's precision")

    # Scale represents the number of digits held after the decimal point.
    if exp < 0:
        if -exp > scale:
            raise ValueError("decimal value requires greater decimal scale than is legal according to the schema")
...
def decode_timestamp_millis(src: IO[bytes]) -> datetime.datetime:
    ...
    return __unix_epoch_start_with_tz + datetime.timedelta(microseconds=total_millis * 1000)
...
def encode_timestamp_millis(msg: datetime.datetime) -> bytes:
    ...
    val = (epoch_delta.days * 24 * 60 * 60 * 1000 + \
...
def encode_timestamp_micros(msg: datetime.datetime) -> bytes:
    if msg.tzinfo is None:
        ...
        epoch_delta = msg - __unix_epoch_start_with_tz
    val = (epoch_delta.days * 24 * 60 * 60 * 1000000 + \
