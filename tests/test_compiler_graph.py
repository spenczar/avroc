...
from avroc.codegen.graph import (
    _schema_to_graph,
    _find_cycle_roots,
...
def test_compute_namegraph_empty_record():
    schema = load_schema({"type": "record", "name": "root", "fields": []})
...
def test_compute_namegraph_nested_record():
    ...
    graph = [NamegraphNode(schema, [NamegraphNode(load_schema(subschema), [])])]
    have = _schema_to_graph(schema, {})
    assert have == graph
...
def test_compute_namegraph_complicated():
    ...
    association_schema = db_schema._names["Association"]
    user_schema = db_schema._names["User"]
